...
export const INCREMENT_TRACK_VIEWS = gql`
  mutation IncrementTrackViewsMutation($incrementTrackViewsId: ID!) {
    incrementViewCount(id: $incrementTrackViewsId) {
      code
      success
      message
      track {
...
const TrackCard = ({ track }) => {
  ...
  // USE_MUTATION's return type is like: [mutationFunction, {loading,err,data}]
  const [mutationFunction] = useMutation(INCREMENT_TRACK_VIEWS, {
    variables: {
  ...
  return (
    <CardContainer to={`/track/${id}`} onClick={mutationFunction}>
      <CardContent>
        <CardImageContainer>
